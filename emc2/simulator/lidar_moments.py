        ...
        for hyd_type in ["pi", "pl", "ci", "cl"]:
            ...
            for tt in range(Dims[2]):
                for k in range(Dims[1]):
                    ...
                    model.ds["sub_col_beta_p_%s_strat" % hyd_type][:, k, tt] = (Calc_tmp[:, :].sum(axis=1)/2 + \
                                                                    Calc_tmp[:, 1:-1].sum(axis=1)) * dD
                    Calc_tmp = np.tile(instrument.mie_table[hyd_type]["alpha_p"], (model.num_subcolumns, 1)) * N_D
                    model.ds["sub_col_alpha_p_%s_strat" % hyd_type][:, k, tt] = (Calc_tmp[:, :].sum(axis=1) / 2 + \
                                                                     Calc_tmp[:, 1:-1].sum(axis=1)) * dD
