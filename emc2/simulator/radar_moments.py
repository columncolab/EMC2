...
def accumulate_attenuation(model, is_conv, z_values, hyd_ext, atm_ext, OD_from_sfc=True,
    ...
    if OD_from_sfc:
        ...
        if not LES_mode:
            ...
            atm_ext = np.cumsum(dz * atm_ext[:, :, :], axis=2)
    else:
...
def calc_radar_bulk(instrument, model, is_conv, p_values, z_values, atm_ext, OD_from_sfc=True,
    ...
    for hyd_type in hyd_types:
        ...
        if np.logical_and(np.isin(hyd_type, ["cl", "pl"]), model.model_name in ["E3SM", "CAM5", "CAM6"]):
            ...
            #interp_vals = interpn((mu_b, lambda_b), Qback_bulk,
            #                      (mu_array[rel_locs], lambda_array[rel_locs]), method="linear")
            ...
            interp_vals = interpolator(mu_array[rel_locs], lambda_array[rel_locs])
            #nterp_vals = interpn((mu_b, lambda_b), Qext_bulk,
            #                     (mu_array[rel_locs], lambda_array[rel_locs]), method="linear")
...
def calc_radar_micro(instrument, model, z_values, atm_ext, OD_from_sfc=True,
    ...
    if LES_mode:
        raise NotImplementedError("Use of microphysics mode not yet implemented with LES mode!")
