...
def test_mie_liquid():
    ...
    np.testing.assert_allclose(KAZR_mie.alpha_p.values, KAZR.mie_table["cl"].alpha_p.values, rtol=1e-1)
    np.testing.assert_allclose(KAZR_mie.beta_p.values, KAZR.mie_table["cl"].beta_p.values, rtol=1e-1)
    np.testing.assert_allclose(KAZR_mie.scat_p.values, KAZR.mie_table["cl"].scat_p.values, rtol=1e-1)
    np.testing.assert_allclose(KAZR_mie.p_diam.values, KAZR.mie_table["cl"].p_diam.values, rtol=1e-1)
